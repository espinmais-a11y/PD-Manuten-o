...
export function ServiceOrderModal({ isOpen, onClose, onSuccess, editingOrder, onCheckOut }: ServiceOrderModalProps) {
  ...
  const tabs: { id: TabId; label: string; icon: React.ElementType; show: boolean }[] = [
    ...
    { id: 'service', label: 'Serviço', icon: Wrench, show: true },
    { id: 'preventive', label: 'Preventiva', icon: ShieldCheck, show: !!showPreventiveTab },
    { id: 'photos', label: 'Fotos', icon: Camera, show: !!showPhotosTab },
    { id: 'signature', label: 'Assinatura', icon: PenLine, show: !!showSignatureTab },
  ].filter(t => t.show);
